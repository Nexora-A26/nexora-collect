...
export function tableHtml(title: string, rows: Record<string, any>[], headers: Record<string,string>, footer = '') {
  ...
  const body = rows.map((r)=>`<tr${r.__isTotal ? ' class="total"' : ''}>${Object.keys(headers).map((k)=>`<td>${escapeHtml(r[k] ?? '')}</td>`).join('')}</tr>`).join('');
  const recordCount = rows.filter((r)=>!r.__isTotal).length;
  return `<h1>${escapeHtml(title)}</h1><div class="meta"><span>تاريخ الإصدار: ${new Date().toLocaleString('ar-IQ')}</span><span>عدد السجلات: ${recordCount}</span></div><table><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table>${footer}`;
}
